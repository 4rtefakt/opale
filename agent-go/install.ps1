...
# --- Détection install pré-rebrand : ancien service TDV-RMM-Agent à
# désinstaller pour éviter la cohabitation. On ne touche PAS au DataDir
# legacy (compat shim côté agent Go), seulement au service Windows.
$LegacyServiceName = 'TDV-RMM-Agent'
if ($ServiceName -ne $LegacyServiceName) {
    $legacy = Get-Service -Name $LegacyServiceName -ErrorAction SilentlyContinue
    if ($legacy) {
        Write-Output "Ancien service $LegacyServiceName détecté — désinstallation"
